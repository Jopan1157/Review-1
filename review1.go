...
func tampilkanData() {
	...
	for i := 0; i < len(data); i++ {
		fmt.Println(i+1, data[i].Kategori, data[i].Jumlah)
	}
}
...
func anggaran() {
	...
	fmt.Println("💰 Total Pengeluaran: Rp", total)
	fmt.Println("🎯 Budget Awal: Rp", totalBudget)

	if total <= totalBudget {
		fmt.Println("✅ Sisa Budget: Rp", totalBudget-total)
	} else {
		fmt.Println("⚠️ Melebihi Budget: Rp", total-totalBudget)
	}
}
